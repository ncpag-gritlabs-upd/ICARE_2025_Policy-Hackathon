...
if __name__ == "__main__":
    ...
    csv_file = "data/sumbong_flood_control/Sumbong_sa_pangulo _FloodControl.csv"
    
    # INPUT: Column(s) to group by - these become the rows of your output
    # Use a single string for one column: 'RegionRes'
    # Use a list for multiple columns: ['RegionRes', 'ProvRes']
    group_columns = ['InfraYear']
    

    
    # INPUT: Dictionary of filters for creating combination counts
    # Keys = column names in your CSV
    # Values = list of values you want to count
    # This will create columns for ALL COMBINATIONS of these values
    filter_dict = {
        'Region': [
            'Region III',
            'Region IV-B',
        ...
        ],  # Column name: list of values to filter
    }
    # The above creates 2×2=4 combination columns:
    # RemovalType_RECOVERED_Sex_MALE, RemovalType_RECOVERED_Sex_FEMALE,
    # RemovalType_DIED_Sex_MALE, RemovalType_DIED_Sex_FEMALE
    
    # INPUT: Dictionary of filters for creating individual counts (optional)
    # Same format as filter_dict, but creates SEPARATE columns for each value
    # Useful when you want totals for each category independently
    extra_single = {
        #'RemovalType': ['RECOVERED', 'DIED'],
        #'Sex': ['MALE', 'FEMALE']
    }
    # The above creates 4 individual columns:
    # RemovalType_RECOVERED, RemovalType_DIED, Sex_MALE, Sex_FEMALE
